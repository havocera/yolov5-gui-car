...
CLASSES = ['Porsche917K_1', 'Ferrari_concept_car', 'Ferrari_458-Italia-GT2']  # coco80类别
isClass = Signal(dict)


# CLASSES = ['electrode', 'breathers', 'ventilate', 'press']
class FooConnection(QtCore.QObject):
    foosignal = QtCore.Signal(dict)
...
class DramRes(QtCore.QObject):
    resSignal = QtCore.Signal(str)
    ...
    def draw(self, image, box_data, url):
        ...
        for box, score, cl in zip(boxes, scores, classes):
            ...
            isclassdata ={"link": url, "class": CLASSES[cl]}
    ...
    def run(self,frame, model, url):
        yolomodel = Yolov5ONNX(model)
        output, or_img = yolomodel.inference(frame)
        ...
        if len(outbox) == 0:
            ...
            drawres = DramRes()
            globals.golobdraw=drawres
            or_img = drawres.draw(or_img, outbox, url)
        return or_img
